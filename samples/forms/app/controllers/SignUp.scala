...
object SignUp extends Controller {
  ...
  val signupForm: Form[User] = Form(
    ...
    mapping(
      ...
      //No "accept" or "confirm" in User!
    )
    // The mapping signature doesn't match the User case class signature,
    // so we have to define custom binding/unbinding functions
    {
      // Binding: Create a User from the mapping result (ignore the second password and the accept field)
      // Part before the arrow doesn't have to exactly match name, but must match order of paramaters
      (usernameq, emailq, passwordsq, profileq, _) => User(usernameq, passwordsq._1, emailq, profileq) //passwords._1 is first password of tuple 
    } 
    {
